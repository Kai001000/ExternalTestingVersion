...
### 2026-09-07 - Market View Weekly Data Update 20260907

Context: weekly Market View data refresh | Streamlit deploy branch data sync | verified existing-DAT manual path | latest-edge density and Market Region coverage check

Source and command:
- Source package: `RawData/1 page update/20260907.zip`
- Source package size: 264,926 bytes
- ZIP archive validation returned `bad_member: None` and found 123 `.DAT` entries.
- DAT destination: `RawData/DAT/2026/20260907`
- DAT files present before update: 123
- DAT files used for rebuild: 123
- The pre-existing DAT directory matched the ZIP by `.DAT` count, filename set, file size, and SHA256.

Run note:
- Pre-run DAT check found `RawData/DAT/2026/20260907` already existed and was non-empty, so the normal wrapper was not used for extraction.
- Verified existing-DAT manual path was used after ZIP/DAT equivalence was confirmed.
- Commands run: `.\.venv\Scripts\python.exe refresh_dat_manifest.py --min_year 2026 --max_year 2026`; `.\.venv\Scripts\python.exe build_fact_sales_year.py --overwrite --min_year 2026 --max_year 2026`; `.\.venv\Scripts\python.exe build_mart_daily_rolling.py`.
- An initial unscoped manifest refresh was discarded because it scanned all years and produced 78,740 rows; the final committed manifest was rebuilt with the same 2026 scope used by the wrapper path.
- The mart build completed with exit code 0.
...
Validated outputs:
- `RawData/manifest/dat_manifest.csv`: 4,378 rows, modified `2026-09-07 18:41:07`
- `Processed/fact_sales/fact_sales_2026.parquet`: 110,884 rows, latest `contract_date` `2026-09-03`, modified `2026-09-07 18:41:22`
- `Processed/mart_daily_rolling/daily_rolling_nsw.parquet`: 12,083 rows, latest date `2026-09-03`, modified `2026-09-07 18:41:32`
- `Processed/mart_daily_rolling/daily_rolling_region.parquet`: 23,759 rows, latest date `2026-09-03`, modified `2026-09-07 18:41:32`
- `Processed/mart_daily_rolling/daily_rolling_region16.parquet`: 207,436 rows, latest date `2026-09-03`, modified `2026-09-07 18:41:33`
- `Processed/mart_daily_rolling/daily_rolling_suburb.parquet`: 9,973,382 rows, latest date `2026-09-03`, modified `2026-09-07 18:41:34`
- `Processed/mart_daily_rolling/daily_rolling_postcode.parquet`: 4,001,934 rows, latest date `2026-09-03`, modified `2026-09-07 18:41:35`

Comparison to 20260831:
- DAT count changed from 125 to 123 for the weekly ZIP batch.
- Manifest rows increased from 4,255 to 4,378.
- `fact_sales_2026` rows increased from 107,539 to 110,884, with 27 rows after previous max `contract_date` `2026-08-27`.
- The total 2026 fact row increase was 3,345 rows: 27 rows on new dates after `2026-08-27` and 3,318 rows of historical backfill at or before `2026-08-27` (99.19% historical backfill).
- Latest `contract_date` advanced from `2026-08-27` to `2026-09-03`; latest day contains 4 fact rows.
- NSW rolling rows increased from 12,069 to 12,083; latest date advanced from `2026-08-27` to `2026-09-03`.
- Region rolling rows increased from 23,731 to 23,759; latest date advanced from `2026-08-27` to `2026-09-03`.
- Market Region/`REGION16` rolling rows increased from 207,168 to 207,436; latest date advanced from `2026-08-27` to `2026-09-03`.
- Suburb rolling rows increased from 9,956,663 to 9,973,382; latest date advanced from `2026-08-27` to `2026-09-03`.
- Postcode rolling rows increased from 3,995,566 to 4,001,934; latest date advanced from `2026-08-27` to `2026-09-03`.
- Duplicate-row checks returned 0 for manifest, fact, and all five `daily_rolling_*` mart files.
- There were 0 fact or mart dates later than the batch date `2026-09-07`.
- The update is predominantly historical registration backfill; the newest contract-date tail remains provisional registration-lag data.

Latest-edge density:
- `2026-08-24`: 9 fact rows
- `2026-08-25`: 17 fact rows
- `2026-08-26`: 8 fact rows
- `2026-08-27`: 10 fact rows
- `2026-08-28`: 9 fact rows
- `2026-08-30`: 1 fact row
- `2026-08-31`: 8 fact rows
- `2026-09-01`: 4 fact rows
- `2026-09-02`: 1 fact row
- `2026-09-03`: 4 fact rows

Region16 check:
- Region16 does not lag NSW/Region in max mart date for this run; all three max dates are `2026-09-03`.
- Statewide fact rows after `2026-08-27`: 27.
- Region16-mapped fact rows after `2026-08-27`: 9.
- Region16-mapped fact max `contract_date`: `2026-09-03`.
- Region16 mart max date matches the mapped fact max date, and there are 0 Region16-mapped fact rows after `2026-09-03`, so there is no stale mart-build indication.
...
- Local Streamlit server smoke used `home.py` on port `8510`; health endpoint returned 200, `/` returned 200, `/Market_View` returned 200, Playwright rendered `/Market_View` with latest date `2026-09-03`, and the server process was stopped with port `8510` released.
- The Playwright page smoke had no Traceback/Exception text.
- Runtime data validation confirmed NSW, Region, Market Region/`REGION16`, Suburb, and Postcode marts load and contain refreshed dates.
...
- `MACQUARIE PARK (2113)` + `HOUSE` + `1 Year` kept the selector on `MACQUARIE PARK (2113)` and did not reset; direct data validation found 176 suburb rows and 4,343 postcode `2113` rows for `HOUSE`.
...
- `RawData/1 page update/20260907.zip`, `RawData/DAT/2026/20260907/`, weekly source zip files, virtualenvs, research folders, cache/temp/log files, and unrelated dirty local files were not staged for deployment.
- Final commit hash is reported in the session handoff after commit creation.

### 2026-08-31 - Market View Weekly Data Update 20260831

Context: weekly Market View data refresh | Streamlit deploy branch data sync | latest-edge density and Market Region coverage check

Source and command:
- Source package: `RawData/1 page update/20260831.zip`
- Source package size: 392,210 bytes
- ZIP archive validation returned `bad_member: None` and found 125 `.DAT` entries.
- Primary command: `.\.venv\Scripts\python.exe scripts\update_market_view_from_zip.py --date 20260831`
- DAT destination: `RawData/DAT/2026/20260831`
- DAT files extracted: 125

Run note:
- Pre-run DAT check found `RawData/DAT/2026/20260831` did not exist, so the normal wrapper path was used.
...
- No Python traceback, native crash, or native-crash recovery command occurred.
- Recovery command `.\.venv\Scripts\python.exe -X faulthandler build_mart_daily_rolling.py` was not needed.
- Post-extraction DAT verification matched the ZIP by `.DAT` count, filename set, file size, and SHA256.

Validated outputs:
- `RawData/manifest/dat_manifest.csv`: 4,255 rows, modified `2026-08-31 19:23:12`
- `Processed/fact_sales/fact_sales_2026.parquet`: 107,539 rows, latest `contract_date` `2026-08-27`, modified `2026-08-31 19:23:15`
- `Processed/mart_daily_rolling/daily_rolling_nsw.parquet`: 12,069 rows, latest date `2026-08-27`, modified `2026-08-31 19:23:17`
- `Processed/mart_daily_rolling/daily_rolling_region.parquet`: 23,731 rows, latest date `2026-08-27`, modified `2026-08-31 19:23:17`
- `Processed/mart_daily_rolling/daily_rolling_region16.parquet`: 207,168 rows, latest date `2026-08-27`, modified `2026-08-31 19:23:18`
- `Processed/mart_daily_rolling/daily_rolling_suburb.parquet`: 9,956,663 rows, latest date `2026-08-27`, modified `2026-08-31 19:23:19`
- `Processed/mart_daily_rolling/daily_rolling_postcode.parquet`: 3,995,566 rows, latest date `2026-08-27`, modified `2026-08-31 19:23:20`

Comparison to 20260824:
- DAT count changed from 127 to 125 for the weekly ZIP batch.
- Manifest rows increased from 4,130 to 4,255.
- `fact_sales_2026` rows increased from 102,058 to 107,539, with 23 rows after previous max `contract_date` `2026-08-20`.
- The total 2026 fact row increase was 5,481 rows: 23 rows on new dates after `2026-08-20` and 5,458 rows of historical backfill at or before `2026-08-20`.
- Latest `contract_date` advanced from `2026-08-20` to `2026-08-27`; latest day contains 4 fact rows.
- NSW rolling rows increased from 12,055 to 12,069; latest date advanced from `2026-08-20` to `2026-08-27`.
- Region rolling rows increased from 23,703 to 23,731; latest date advanced from `2026-08-20` to `2026-08-27`.
- Market Region/`REGION16` rolling rows increased from 206,847 to 207,168; latest date advanced from `2026-08-19` to `2026-08-27`.
- Suburb rolling rows increased from 9,940,112 to 9,956,663; latest date advanced from `2026-08-20` to `2026-08-27`.
- Postcode rolling rows increased from 3,989,462 to 3,995,566; latest date advanced from `2026-08-20` to `2026-08-27`.
- Duplicate-row checks returned 0 for manifest, fact, and all five `daily_rolling_*` mart files.
- The update is predominantly historical registration backfill; the newest contract-date tail remains provisional registration-lag data.

Latest-edge density:
- `2026-08-19`: 9 fact rows
- `2026-08-20`: 11 fact rows
- `2026-08-21`: 6 fact rows
- `2026-08-24`: 7 fact rows
- `2026-08-25`: 4 fact rows
- `2026-08-26`: 2 fact rows
- `2026-08-27`: 4 fact rows

Region16 check:
- Region16 no longer lags NSW/Region in max mart date for this run; all three max dates are `2026-08-27`.
- Statewide fact rows after `2026-08-19`: 34 total after mart filters.
- Region16-mapped fact rows after `2026-08-19`: 4.
- Region16-mapped fact max `contract_date`: `2026-08-27`.
- Region16 mart max date matches the mapped fact max date, and there are 0 Region16-mapped fact rows after `2026-08-27`, so there is no stale mart-build indication.

Validation:
- `.\.venv\Scripts\python.exe -m pytest` was attempted but the `.venv` environment does not have `pytest` installed.
- `.\.venv\Scripts\python.exe -m unittest tests.test_market_view_area_selection` passed: 8 tests.
- `py_compile` passed for `home.py`, `pages/1_Market_View.py`, `utils/data.py`, `utils/i18n.py`, `utils/market_view_area.py`, `utils/tables.py`, and `tests/test_market_view_area_selection.py`.
- Local Streamlit server smoke used `home.py` on port `8510`; health endpoint returned 200, `/` returned 200, `/Market_View` returned 200, Playwright rendered `/Market_View` with latest date `2026-08-27`, and the server process was stopped with port `8510` released.
- The Playwright page smoke had no Traceback/Exception text, no page errors, and no severe console errors; non-fatal console noise was limited to existing 404/theme warnings.
- Runtime data validation confirmed NSW, Region, Market Region/`REGION16`, Suburb, and Postcode marts load and contain refreshed dates.
- Real-data selector validation confirmed saved state `Macquarie Park (2113)` is coerced to existing option `MACQUARIE PARK (2113)`.
...
- Branch: `deploy/streamlit-cloud-safe-2026-03-29`
- Active deploy data files are the tracked `Processed/fact_sales/`, `Processed/mart_daily_rolling/`, and `RawData/manifest/dat_manifest.csv` artifacts.
- `RawData/1 page update/20260831.zip`, `RawData/DAT/2026/20260831/`, weekly source zip files, virtualenvs, cache/temp files, and unrelated dirty local files were not staged for deployment.
- Final commit hash is reported in the session handoff after commit creation.
...
Source and command:
- Source package: `RawData/1 page update/20260810.zip`
- Source package size: 213,986 bytes
- Primary command: `.\.venv\Scripts\python.exe scripts\update_market_view_from_zip.py --date 20260810`
- DAT destination: `RawData/DAT/2026/20260810`
- DAT files extracted: 123

Run note:
- Pre-run DAT check found `RawData/DAT/2026/20260810` did not exist, so the normal wrapper path was used.
- The ZIP was readable, contained a single expected 20260810 package, and contained 123 `.DAT` files.
- The primary wrapper completed extraction, manifest refresh, `fact_sales_2026` rebuild, and daily rolling mart rebuild with exit code 0.
- No Python traceback, native crash, or native-crash recovery command occurred.
- Recovery command `.\.venv\Scripts\python.exe -X faulthandler build_mart_daily_rolling.py` was not needed.

Validated outputs:
- `RawData/manifest/dat_manifest.csv`: 3,882 rows, modified `2026-08-10 19:57:38`
- `Processed/fact_sales/fact_sales_2026.parquet`: 96,466 rows, latest `contract_date` `2026-08-05`, modified `2026-08-10 19:57:41`
- `Processed/mart_daily_rolling/daily_rolling_nsw.parquet`: 12,025 rows, latest date `2026-08-05`, modified `2026-08-10 19:57:43`
- `Processed/mart_daily_rolling/daily_rolling_region.parquet`: 23,643 rows, latest date `2026-08-05`, modified `2026-08-10 19:57:44`
- `Processed/mart_daily_rolling/daily_rolling_region16.parquet`: 206,199 rows, latest date `2026-08-03`, modified `2026-08-10 19:57:44`
- `Processed/mart_daily_rolling/daily_rolling_suburb.parquet`: 9,909,925 rows, latest date `2026-08-05`, modified `2026-08-10 19:57:45`
- `Processed/mart_daily_rolling/daily_rolling_postcode.parquet`: 3,976,924 rows, latest date `2026-08-05`, modified `2026-08-10 19:57:46`

Comparison to 20260803:
- DAT count remained 123 for the weekly ZIP batch.
- Manifest rows increased from 3,759 to 3,882.
- `fact_sales_2026` rows increased from 93,875 to 96,466, with 19 rows after previous max `contract_date` `2026-07-30`.
- Latest `contract_date` advanced from `2026-07-30` to `2026-08-05`; latest day contains 1 fact row.
- NSW rolling rows increased from 12,013 to 12,025; latest date advanced from `2026-07-30` to `2026-08-05`.
- Region rolling rows increased from 23,619 to 23,643; latest date advanced from `2026-07-30` to `2026-08-05`.
- Market Region/`REGION16` rolling rows increased from 205,804 to 206,199; latest date advanced from `2026-07-24` to `2026-08-03`.
- Suburb rolling rows increased from 9,895,352 to 9,909,925; latest date advanced from `2026-07-30` to `2026-08-05`.
- Postcode rolling rows increased from 3,971,409 to 3,976,924; latest date advanced from `2026-07-30` to `2026-08-05`.
- Duplicate-row checks returned 0 for manifest, fact, and all five `daily_rolling_*` mart files.
- Latest post-`2026-07-30` fact rows are very thin; treat newest days as provisional registration-lag data rather than a stable trend signal.

Region16 check:
- Region16 still lags NSW/Region in max mart date: Region16 is `2026-08-03`, while NSW and Region are `2026-08-05`.
- Statewide 2026 fact rows after `2026-07-24`: 60 total.
- Region16-mapped 2026 fact rows after `2026-07-24`: 12.
- Region16-mapped fact max `contract_date`: `2026-08-03`.
- Region16 mart max date matches the mapped fact max date, so the lag is explained by mapped coverage and not by a stale mart build.

Validation:
- `.\.venv\Scripts\python.exe -m pytest` was attempted but the `.venv` environment does not have `pytest` installed.
- `.\.venv\Scripts\python.exe -m unittest tests.test_market_view_area_selection` passed: 8 tests.
- `py_compile` passed for `home.py`, `pages/1_Market_View.py`, `utils/data.py`, `utils/i18n.py`, `utils/market_view_area.py`, `utils/tables.py`, and `tests/test_market_view_area_selection.py`.
- Runtime data-load smoke confirmed NSW, Region, Market Region/`REGION16`, Suburb, and Postcode marts load through `utils.data.load_daily_rolling`.
- Streamlit AppTest rendered `pages/1_Market_View.py` with 0 exceptions, 0 error elements, latest date `2026-08-05` visible, period `1 Year`, level `AREA`, and selector value `MACQUARIE PARK (2113)`.
- Local Streamlit server smoke used `home.py` on port `8510`; health endpoint returned 200, `/` returned 200, `/Market_View` returned 200, logs had no Traceback/Exception text, and the server process was stopped after validation.
- Real-data selector validation confirmed saved state `Macquarie Park (2113)` is coerced to existing option `MACQUARIE PARK (2113)`.
- `MACQUARIE PARK (2113)` + `HOUSE` + `1 Year` kept the selector on `MACQUARIE PARK (2113)` and produced an empty filtered result, matching the expected empty-state path.

Deployment sync:
- Branch: `deploy/streamlit-cloud-safe-2026-03-29`
- Active deploy data files are the tracked `Processed/fact_sales/`, `Processed/mart_daily_rolling/`, and `RawData/manifest/dat_manifest.csv` artifacts.
- `data/public_market_view/` remains a legacy snapshot layer and was not updated for this run.
- `RawData/1 page update/20260810.zip`, `RawData/DAT/2026/20260810/`, weekly source zip files, virtualenvs, cache/temp files, and unrelated dirty local files were not staged for deployment.
- Final commit hash is reported in the session handoff after commit creation.

### 2026-08-03 - Market View Weekly Data Update 20260803
...
- Primary command attempted: `.\.venv\Scripts\python.exe scripts\update_market_view_from_zip.py --date 20260803`
- DAT destination: `RawData/DAT/2026/20260803`
- DAT files present: 123

Run note:
- The primary wrapper selected the expected ZIP and target date, then stopped before extraction with exit code 1 because `RawData/DAT/2026/20260803` already existed and was non-empty.
- No Python traceback and no native exit occurred.
- ZIP-to-DAT verification found 123 `.DAT` files in the ZIP and 123 `.DAT` files in the destination, with matching names and sizes.
- To avoid clearing the existing DAT directory, the wrapper's internal build steps were run directly:
...
- All three direct build steps completed with exit code 0.
- Recovery command `.\.venv\Scripts\python.exe -X faulthandler build_mart_daily_rolling.py` was not needed.
...
- DAT count remained 123 for the weekly ZIP batch.
- Manifest rows increased from 3,636 to 3,759.
- `fact_sales_2026` rows increased from 91,071 to 93,875, with 20 rows after previous max `contract_date` `2026-07-23`.
- Latest `contract_date` advanced from `2026-07-23` to `2026-07-30`; latest day contains 1 fact row.
- NSW rolling rows increased from 11,999 to 12,013; latest date advanced from `2026-07-23` to `2026-07-30`.
- Region rolling rows increased from 23,591 to 23,619; latest date advanced from `2026-07-23` to `2026-07-30`.
- Market Region/`REGION16` rolling rows increased from 205,735 to 205,804; latest date advanced from `2026-07-23` to `2026-07-24`.
- Suburb rolling rows increased from 9,879,654 to 9,895,352; latest date advanced from `2026-07-23` to `2026-07-30`.
- Postcode rolling rows increased from 3,964,927 to 3,971,409; latest date advanced from `2026-07-23` to `2026-07-30`.
- Duplicate-row checks returned 0 for manifest, fact, and all five `daily_rolling_*` mart files.
- Latest post-`2026-07-23` fact rows are very thin; treat newest days as provisional registration-lag data rather than a stable trend signal.

Region16 check:
- Region16 lags NSW/Region in max mart date: Region16 is `2026-07-24`, while NSW and Region are `2026-07-30`.
- Statewide 2026 fact rows after `2026-07-23`: 20 total; 20 after mart filters.
- Region16-mapped 2026 fact rows after `2026-07-23`: 2.
- Region16-mapped fact max `contract_date`: `2026-07-24`.
- Region16 mart max date matches the mapped fact max date, so the lag is explained by mapped coverage and not by a stale mart build.
...
- `.\.venv\Scripts\python.exe -m unittest tests.test_market_view_area_selection` passed: 8 tests.
- `py_compile` passed for `home.py`, `pages/1_Market_View.py`, `utils/data.py`, `utils/i18n.py`, `utils/market_view_area.py`, `utils/tables.py`, and `tests/test_market_view_area_selection.py`.
- Streamlit AppTest rendered NSW, Region, Market Region/`REGION16`, and `MACQUARIE PARK (2113)` AREA saved-state cases with 0 exceptions; rendered text included latest date `2026-07-30`.
- Local Streamlit server smoke used `home.py` on port `8510`; health endpoint returned 200, `/` returned 200, `/Market_View` returned 200, both page bodies had no Traceback/Exception text, and the server process was stopped after validation.
- Real-data selector validation confirmed saved state `Macquarie Park (2113)` is coerced to existing option `MACQUARIE PARK (2113)`.
- `MACQUARIE PARK (2113)` + `HOUSE` + `1 Year` kept the selector on `MACQUARIE PARK (2113)` and produced an empty filtered result, matching the expected empty-state path.
...
- Active deploy data files are the tracked `Processed/fact_sales/`, `Processed/mart_daily_rolling/`, and `RawData/manifest/dat_manifest.csv` artifacts.
- `data/public_market_view/` remains a legacy snapshot layer and was not updated for this run.
- `RawData/1 page update/20260803.zip`, `RawData/DAT/2026/20260803/`, weekly source zip files, virtualenvs, cache/temp files, and unrelated dirty local files were not staged for deployment.
- Final commit hash is reported in the session handoff after commit creation.

### 2026-07-27 - Market View Weekly Data Update 20260727

Context: weekly Market View data refresh | Streamlit deploy branch data sync | selector empty-state regression check

Source and command:
- Source package: `RawData/1 page update/20260727.zip`
- Primary command: `.\.venv\Scripts\python.exe scripts\update_market_view_from_zip.py --date 20260727`
- DAT destination: `RawData/DAT/2026/20260727`
- DAT files extracted: 123

Run note:
- The primary wrapper completed ZIP extraction, manifest refresh, `fact_sales_2026` rebuild, and `build_mart_daily_rolling` successfully with exit code 0.
- No native crash occurred. Recovery command `.\.venv\Scripts\python.exe -X faulthandler build_mart_daily_rolling.py` was not needed.
...
- `Processed/mart_daily_rolling/daily_rolling_nsw.parquet`: 11,999 rows, latest date `2026-07-23`
- `Processed/mart_daily_rolling/daily_rolling_region.parquet`: 23,591 rows, latest date `2026-07-23`
- `Processed/mart_daily_rolling/daily_rolling_region16.parquet`: 205,735 rows, latest date `2026-07-23`
- `Processed/mart_daily_rolling/daily_rolling_suburb.parquet`: 9,879,654 rows, latest date `2026-07-23`
- `Processed/mart_daily_rolling/daily_rolling_postcode.parquet`: 3,964,927 rows, latest date `2026-07-23`

Comparison to 20260720:
- DAT count changed from 127 to 123 for the weekly ZIP batch.
- Manifest rows increased from 3,513 to 3,636.
- `fact_sales_2026` rows increased from 88,519 to 91,071, with 15 rows after previous max `contract_date` `2026-07-16`.
- Latest `contract_date` advanced from `2026-07-16` to `2026-07-23`; latest day contains 3 fact rows.
- NSW rolling rows increased from 11,985 to 11,999; latest date advanced from `2026-07-16` to `2026-07-23`.
- Region rolling rows increased from 23,563 to 23,591; latest date advanced from `2026-07-16` to `2026-07-23`.
- Market Region/`REGION16` rolling rows increased from 205,170 to 205,735; latest date advanced from `2026-07-09` to `2026-07-23`.
- Suburb rolling rows increased from 9,863,975 to 9,879,654; latest date advanced from `2026-07-16` to `2026-07-23`.
- Postcode rolling rows increased from 3,958,728 to 3,964,927; latest date advanced from `2026-07-16` to `2026-07-23`.
- Duplicate-row checks returned 0 for manifest, fact, and all five `daily_rolling_*` mart files.
- Latest post-`2026-07-16` fact rows are very thin; treat newest days as provisional registration-lag data rather than a stable trend signal.

Region16 check:
- Region16 is not lagging NSW/Region in this run; all three max dates are `2026-07-23`.
- 2026 statewide fact rows after `2026-07-09`: 68; Region16-mapped rows: 14; mapped max `contract_date`: `2026-07-23`.
- 2026 statewide fact rows after `2026-07-16`: 15; Region16-mapped rows: 4; mapped max `contract_date`: `2026-07-23`.
- Region16 mart max date matches mapped fact max date, so there is no indication of a stale mart build.
...
- Local Streamlit server smoke used `home.py` on port `8510`; health endpoint returned 200, `/` loaded, `/Market_View` loaded without Traceback/Exception text, and the server process was stopped after validation.
- Real-data selector validation confirmed saved state `Macquarie Park (2113)` is coerced to existing option `MACQUARIE PARK (2113)`.
...
- Active deploy data files are the tracked `Processed/fact_sales/`, `Processed/mart_daily_rolling/`, and `RawData/manifest/dat_manifest.csv` artifacts.
- `data/public_market_view/` remains a legacy snapshot layer and was not updated for this run.
- `RawData/1 page update/20260727.zip`, `RawData/DAT/2026/20260727/`, weekly source zip files, virtualenvs, cache/temp files, and unrelated dirty local files were not staged for deployment.
- Final commit hash is reported in the session handoff after commit creation.
...
- Command: `.\.venv\Scripts\python.exe scripts\update_market_view_from_zip.py --date 20260713`
- DAT destination: `RawData/DAT/2026/20260713`
- DAT files extracted: 127
...
- `Processed/mart_daily_rolling/daily_rolling_nsw.parquet`: 11,971 rows, latest date `2026-07-09`
- `Processed/mart_daily_rolling/daily_rolling_region.parquet`: 23,535 rows, latest date `2026-07-09`
- `Processed/mart_daily_rolling/daily_rolling_region16.parquet`: 205,063 rows, latest date `2026-07-07`
- `Processed/mart_daily_rolling/daily_rolling_suburb.parquet`: 9,849,316 rows, latest date `2026-07-09`
- `Processed/mart_daily_rolling/daily_rolling_postcode.parquet`: 3,952,868 rows, latest date `2026-07-09`

Comparison to 20260706:
- DAT count increased from 126 to 127.
- Manifest rows increased from 3,259 to 3,386.
- `fact_sales_2026` rows increased from 83,176 to 86,050, with 30 rows after previous max `contract_date` `2026-07-01`.
- NSW rolling rows increased from 11,955 to 11,971; latest date advanced from `2026-07-01` to `2026-07-09`.
- Region rolling rows increased from 23,503 to 23,535; latest date advanced from `2026-07-01` to `2026-07-09`.
- Market Region/`REGION16` rolling rows increased from 204,763 to 205,063; latest date advanced from `2026-06-30` to `2026-07-07`.
- Region16 lag is consistent with mapped underlying fact coverage: Region16-mapped 2026 fact rows after `2026-07-01` total 5 and have max `contract_date` `2026-07-07`.
- Latest post-`2026-07-01` fact rows are thin; treat newest days as provisional registration-lag data rather than a stable trend signal.
...
- `.\.venv\Scripts\python.exe -m unittest tests.test_market_view_area_selection` passed: 8 tests.
- `py_compile` passed for `home.py`, `pages/1_Market_View.py`, `utils/data.py`, `utils/i18n.py`, `utils/market_view_area.py`, `utils/tables.py`, and `tests/test_market_view_area_selection.py`.
- Local Streamlit server smoke used `home.py` on port `8510`; health endpoint returned 200, `/` loaded, `/Market_View` loaded, and rendered text showed data as of `2026-07-09`; the server job was stopped after validation.
- Real-data regression validation found the actual area option label is `MACQUARIE PARK (2113)`. `coerce_selected_area_label()` now matches existing options case-insensitively so saved `Macquarie Park (2113)` state does not reset to the first area option.
- `MACQUARIE PARK (2113)` + `HOUSE` + `1 Year` kept the selector on `MACQUARIE PARK (2113)` and produced an empty filtered result, matching the expected empty state.
...
- Active deploy data files are the tracked `Processed/fact_sales/`, `Processed/mart_daily_rolling/`, and `RawData/manifest/dat_manifest.csv` artifacts.
- `data/public_market_view/` remains a legacy snapshot layer and was not updated for this run.
- `RawData/1 page update/20260713.zip`, `RawData/DAT/`, weekly source zip files, virtualenvs, cache/temp files, and unrelated dirty local files were not staged for deployment.
- Final commit hash is reported in the session handoff after commit creation.

### 2026-07-06 — Market View Weekly Data Update 20260706

Context: weekly Market View data refresh | Streamlit deploy branch data sync | selector empty-state regression check

Source and command:
- Source package: `RawData/1 page update/20260706.zip`
- Command: `.\.venv\Scripts\python.exe scripts\update_market_view_from_zip.py --date 20260706`
- DAT destination: `RawData/DAT/2026/20260706`
...
- `Processed/mart_daily_rolling/daily_rolling_nsw.parquet`: 11,955 rows, latest date `2026-07-01`
- `Processed/mart_daily_rolling/daily_rolling_region.parquet`: 23,503 rows, latest date `2026-07-01`
- `Processed/mart_daily_rolling/daily_rolling_region16.parquet`: 204,763 rows, latest date `2026-06-30`
- `Processed/mart_daily_rolling/daily_rolling_suburb.parquet`: 9,834,425 rows, latest date `2026-07-01`
- `Processed/mart_daily_rolling/daily_rolling_postcode.parquet`: 3,946,820 rows, latest date `2026-07-01`

Comparison to 20260629:
- DAT count decreased from 127 to 126.
- Manifest rows increased from 3,133 to 3,259.
- `fact_sales_2026` rows increased from 79,472 to 83,176, with 55 rows after previous max `contract_date` `2026-06-25`.
- NSW rolling rows increased from 11,943 to 11,955; latest date advanced from `2026-06-25` to `2026-07-01`.
- Region rolling rows increased from 23,479 to 23,503; latest date advanced from `2026-06-25` to `2026-07-01`.
- Market Region/`REGION16` rolling rows increased from 204,556 to 204,763; latest date advanced from `2026-06-25` to `2026-06-30`.
- The latest post-`2026-06-25` fact rows are thin; treat the newest few days as provisional registration-lag data rather than a stable trend signal.
...
- Streamlit AppTest rendered NSW, Region, and Market Region/`REGION16` views without exceptions; `load_daily_rolling()` read latest dates `2026-07-01`, `2026-07-01`, and `2026-06-30` respectively.
- Local Streamlit server smoke used `home.py` on port `8510`; health endpoint returned 200, `/` returned 200, and `/Market_View` returned 200 before the server was stopped.
- `MACQUARIE PARK (2113)` + `HOUSE` + `1 Year` kept `MACQUARIE PARK (2113)` selected and rendered the `market_view_no_selection_data` empty state.

Deployment sync:
- Branch: `deploy/streamlit-cloud-safe-2026-03-29`
- Active deploy data files are the tracked `Processed/fact_sales/`, `Processed/mart_daily_rolling/`, and `RawData/manifest/dat_manifest.csv` artifacts.
- `data/public_market_view/` remains a legacy snapshot layer and was not updated for this run.
- `RawData/1 page update/20260706.zip`, `RawData/DAT/`, weekly source zip files, virtualenvs, cache/temp files, and unrelated dirty source/doc files were not staged for deployment.
- Final commit hash is reported in the session handoff after commit creation.

### 2026-06-29 — Market View Weekly Data Update 20260629

Context: weekly Market View data refresh | Streamlit deploy branch data sync | selector empty-state regression check

Source and command:
- Source package: `RawData/1 page update/20260629.zip`
- Command: `.\.venv\Scripts\python.exe scripts\update_market_view_from_zip.py --date 20260629`
- DAT destination: `RawData/DAT/2026/20260629`
- DAT files extracted: 127
...
- `Processed/mart_daily_rolling/daily_rolling_nsw.parquet`: 11,943 rows, latest date `2026-06-25`
- `Processed/mart_daily_rolling/daily_rolling_region.parquet`: 23,479 rows, latest date `2026-06-25`
- `Processed/mart_daily_rolling/daily_rolling_region16.parquet`: 204,556 rows, latest date `2026-06-25`
- `Processed/mart_daily_rolling/daily_rolling_suburb.parquet`: 9,815,097 rows, latest date `2026-06-25`
- `Processed/mart_daily_rolling/daily_rolling_postcode.parquet`: 3,939,821 rows, latest date `2026-06-25`

Comparison to 20260622:
- DAT count increased from 123 to 127.
- Manifest rows increased from 3,006 to 3,133.
- `fact_sales_2026` rows increased from 74,725 to 79,472, with 41 rows after previous max `contract_date` `2026-06-18`.
- NSW rolling rows increased from 11,929 to 11,943; latest date advanced from `2026-06-18` to `2026-06-25`.
- Region rolling rows increased from 23,451 to 23,479; latest date advanced from `2026-06-18` to `2026-06-25`.
- Market Region/`REGION16` rolling rows increased from 204,238 to 204,556; latest date advanced from `2026-06-18` to `2026-06-25`.
...
- `.\.venv\Scripts\python.exe -m unittest tests.test_market_view_area_selection` passed: 7 tests.
- `py_compile` passed for `home.py`, `pages/1_Market_View.py`, `utils/data.py`, `utils/i18n.py`, `utils/market_view_area.py`, `scripts/update_market_view_from_zip.py`, `build_fact_sales_year.py`, `refresh_dat_manifest.py`, and `build_mart_daily_rolling.py`.
- Streamlit AppTest loaded Market View with no exceptions for NSW, Region, and Market Region/`REGION16`; latest date `2026-06-25` was visible in rendered page text.
- Local Streamlit server smoke used `home.py` on port `8510`; health endpoint returned 200 and `http://127.0.0.1:8510` returned 200 before the server was stopped.
- `MACQUARIE PARK (2113)` + `HOUSE` + `1 Year` kept `MACQUARIE PARK (2113)` selected and rendered `当前选择在现有筛选条件下暂无数据。`

Deployment sync:
- Branch: `deploy/streamlit-cloud-safe-2026-03-29`
- Active deploy data files are the tracked `Processed/fact_sales/`, `Processed/mart_daily_rolling/`, and `RawData/manifest/dat_manifest.csv` artifacts.
- `RawData/1 page update/20260629.zip`, `RawData/DAT/`, weekly source zip files, virtualenvs, cache/temp files, and unrelated dirty source/doc files were not staged for deployment.
- Final commit hash is reported in the session handoff after commit creation.
